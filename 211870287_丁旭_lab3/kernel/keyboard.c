...
{
	u8 scan_code = in_byte(KB_DATA); // 从键盘数据端口0x60（KB_DATA）读取一个字节
...
/*======================================================================*
						   init_keyboard
*======================================================================*/
PUBLIC void init_keyboard() // 初始化键盘
{
	kb_in.count = 0;
...
PUBLIC void keyboard_read(TTY *p_tty)
{
	...
	if (kb_in.count > 0)
	{
		...
		if ((key != PAUSEBREAK) && (key != PRINTSCREEN))
		{
			...
			/* 先定位到 keymap 中的行 */
			keyrow = &keymap[(scan_code & 0x7F) * MAP_COLS]; // 与0x7F一方面是如果为breakcode 则转变为makecode 另一方面是避免越界。
...
get_byte_from_kbuf() /* 从键盘缓冲区中读取下一个字节 */
{
	u8 scan_code;
...
PRIVATE void kb_wait() /* 等待 8042 的输入缓冲区空 */
{
	...
	} while (kb_stat & 0x02); // 状态寄存器第一位如果是0 则表明8042输入缓冲区为空 可以写入
}
...
PRIVATE void set_leds()
{
	u8 leds = (caps_lock << 2) | (num_lock << 1) | scroll_lock; //  ********  ——>  ******(caps_lock) (num_lock) (scroll_lock)

	kb_wait();
	out_byte(KB_DATA, LED_CODE);
	kb_ack(); // 键盘收到设置led的命令0XED
